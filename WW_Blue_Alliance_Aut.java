...
public class WW_Blue_Alliance_Aut extends SynchronousOpMode
{
    ...
    void goStraightKeepStraight(double TargetDistance)
    {
        ...
        if (Distancedifference > 0)
        {
            if (MaxDrivePowerAchieved == false)// startup power
            {
                ...
                AdjusteddrivePower = 0.07+(Math.pow(X_Position_Inches+2,3))/5000.0;
            }
            ...
            if ((TimeElapsed != 0) && (velocity.velocX <= 0.05))
            {
                if ((ms-TimeElapsed)> 1000)
                {
    ...
    {
        double NewTurningSpeed = SlowTurning();
...
/*        if (PitchReading>= 0.0)
        {
            if (OffsetValue >= 0.0) AdjustedPitch = Math.abs(PitchReading-OffsetValue);
            else AdjustedPitch = (PitchReading+OffsetValue);// OffstVal is negative
        }
        else
        {
            if (OffsetValue >= 0.0) AdjustedPitch = PitchReading+OffsetValue;// PitchReading is negative
            else AdjustedPitch = (PitchReading-OffsetValue);// both are negative
        }*/
        //if (PitchReading > TurningSpeed) AdjustedPitch = TurningSpeed;
